...
import { IUpdateUrlService, UpdateShortUrlResponse, UpdateUrlParams } from "../interfaces/services/update-url-service.interface";
...
export class UpdateUrlService implements IUpdateUrlService {
  ...
  async execute(params: UpdateUrlParams, userId: string): Promise<UpdateShortUrlResponse> {
    const { data, id } = params
    const url = await this.getUrlService.execute({ id })

    if (url.userId !== userId) {
      throw new UnauthorizedError('Você não tem permissão para atualizar esta URL');
    }

    const updatedUrl = await this.updateUrlRepo.update(id, data)
    if (!updatedUrl) throw new BadRequestError('Erro ao atualizar url de origem')
